...
@section('content')
    ...
    <div class="card card-custom">
        ...
        @if($model->id > 0)
            {!! Form::open(['url' => route('users.update',$model->id), 'id'=>'validate', 'method' => 'put']) !!}
        @else
            {!! Form::open(['url' => route('users.store'), 'id' => 'validate']) !!}
        @endif
        <div class="card-body">
            ...
            <div class="form-group row">
                {!! Form::label('roles[]','Perfil',['class' => 'col-lg-3 col-xl-2 col-form-label text-right']) !!}
                <div class="col-lg-9">
                    <div class="input-group">
                        {!! Form::select('roles[]', $roles,$userRole, ['id' => 'roles', 'class' => 'form-control validate[required]']) !!}
                    </div>
                </div>
            </div>
        </div>
        <div class="card-footer">
            ...
            <a href="{!! route('users.index') !!}" class="btn btn-danger font-weight-bolder text-uppercase">Voltar</a>
        </div>
        {!! Form::close() !!}
    </div>
